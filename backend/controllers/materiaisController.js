...
const listarMateriais = async (req, res) => {
    ...
    console.log('trilha_id recebido no controller:', trilha_id);
    console.log('req.query:', req.query);
    console.log('trilha_id:', req.query.trilha_id);
    ...
    try {
        ...
        const whereClause = { trilha_id: Number(trilha_id) };

        const cartas = await Carta.findAll({ where: whereClause, attributes: ['id', 'name', 'trilha_id'], raw: true });
        const videos = await Video.findAll({ where: whereClause, attributes: ['id', 'titulo', 'trilha_id'], raw: true });
        const pdfs = await Pdf.findAll({ where: whereClause, attributes: ['id', 'titulo', 'trilha_id'], raw: true });
